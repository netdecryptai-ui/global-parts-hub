...
// FIX: Use standard import. Note the path has two dots (..) to go back to 'app'
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-8 relative z-20">
        <div className="bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden">
           ...
           <div className="grid grid-cols-3 p-6 border-b items-center">
              <img src={`/api/image?url=${encodeURIComponent(phoneA.image)}`} className="w-full h-32 object-contain" alt={phoneA.name} />
              <div className="text-center text-xs font-bold text-slate-300">IMAGES</div>
              <img src={`/api/image?url=${encodeURIComponent(phoneB.image)}`} className="w-full h-32 object-contain" alt={phoneB.name} />
           </div>
