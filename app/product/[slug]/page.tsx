...
// Try to import data safely. If it fails, we use an empty array.
let phoneDatabase: any[] = [];
try {
  phoneDatabase = require("../../data/phones.json");
} catch (e) {
  console.log("⚠️ phones.json not found. Using AI mode only.");
  phoneDatabase = [];
}
...
async function generateArticleData(slug: string) {
  ...
  // STEP 2: FALLBACK TO AI (If not in Vault)
  console.log(`🤖 Real data not found for ${slug}. Activating AI...`);
  const rawName = slug.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
  
  // Studio Style AI Image
  ...
  try {
    ...
    const result = await model.generateContent(`
      Write a product description for: "${rawName}". 
      Include: Estimated Price, Difficulty Level (Easy/Medium/Hard), and Tools Needed.
      Format: Use HTML tags like <h2> for headings and <p> for paragraphs. Do not use Markdown (**).
    `);
    ...
    return {
      title: rawName,
      content: "<p>Details currently unavailable. Please check Amazon for live pricing.</p>",
      price: "Check Price",
      image: aiImage,
      amazonLink: `https://www.amazon.com/s?k=${slug.replace(/-/g, "+")}&tag=stanley05-20`,
      isRealData: false
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row">
           ...
           <div className="md:w-1/2 bg-white p-8 flex items-center justify-center border-r border-slate-100">
              <img src={data.image} alt={data.title} className="max-h-80 object-contain hover:scale-105 transition" />
           </div>
           ...
           <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
              ...
              <a 
                ...
                target="_blank" 
                className="w-full bg-[#FF9900] hover:bg-[#ffad33] text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 shadow-lg transition transform hover:-translate-y-1"
              >
              ...
              {!data.isRealData && (
                 <p className="text-xs text-slate-400 mt-4 text-center">
                   *Image and price generated by AI. Verify on Amazon.
                 </p>
              )}
           </div>
        </div>
